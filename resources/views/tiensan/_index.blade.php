...
<div class="page-head">
    ...
    <button type="button" class="btn btn-primary" onclick="openTienSanModal()"><i class="ri-add-line"></i> Tạo Tiền sân</button>
</div>
...
<div class="table-card">
    ...
    <table>
        <thead><tr><th>Ngày</th><th>Cơ sở</th><th>Số tiền</th><th>Ghi chú</th><th></th></tr></thead>
        <tbody>
                    ...
                    <td>
                        <div class="actions-cell">
                            <i class="ri-edit-line"
                               onclick='openTienSanModal(@json($ts))'></i>
                            <form action="{{ route('tiensan.destroy', $ts) }}" method="POST" style="display:inline;"
                                  class="confirm-delete-form"
                                  data-confirm-title="Xoá bản ghi Tiền sân"
                                  data-confirm-message="Bạn có chắc muốn xoá bản ghi ngày {{ $ts->ngay->format('d/m/Y') }} của Cơ sở {{ $ts->coSo->ten }}?">
                                @csrf @method('DELETE')
                                <button type="submit" style="background:none;border:none;padding:0;"><i class="ri-delete-bin-line del"></i></button>
                            </form>
                        </div>
                    </td>
                </tr>
            @empty
                <tr><td colspan="5" class="text-2" style="text-align:center;padding:30px;">Chưa có bản ghi Tiền sân nào</td></tr>
            @endforelse
        </tbody>
    </table>
    ...
    <div class="pagination">
        ...
        @for ($i = 1; $i <= $tienSans->lastPage(); $i++)
            <a href="{{ $tienSans->url($i) }}" class="{{ $i == $tienSans->currentPage() ? 'active' : '' }}">{{ $i }}</a>
        @endfor
