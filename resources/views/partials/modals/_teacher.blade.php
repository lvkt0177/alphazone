<div class="overlay" id="teacherModal">
    <div class="modal">
        <div class="modal-head">
            <h3 id="teacherModalTitle">Tạo Giáo viên</h3><i class="ri-close-line" onclick="closeModal('teacherModal')"></i>
        </div>

        <form id="teacherForm" method="POST" action="{{ route('giaovien.store') }}"
            data-store-url="{{ route('giaovien.store') }}" data-update-url-base="{{ url('giaovien') }}">
            @csrf
            <div id="teacherMethodField"></div>
            <input type="hidden" name="_editing_id" id="teacherEditingId">
