...
class SoDoRenderer
{
    public static function render(?array $soDo, array $mauSac, array $kichThuoc = []): string
    {
        if (! $soDo) {
            return '';
        }

        $kichThuoc = array_merge(\App\Models\SodoMauSac::macDinhKichThuoc(), $kichThuoc);

        $html = '';

        foreach ($soDo['objects'] ?? [] as $obj) {
            $html .= self::veVatDung($obj, $mauSac, $kichThuoc);
        }
    ...
    private static function tySo(string $nhom, array $kichThuoc): float
    {
        return ($kichThuoc[$nhom] ?? 100) / 100;
    }

    private static function veVatDung(array $obj, array $mauSac, array $kichThuoc): string
    {
        $x = $obj['x'] ?? 0;
        $y = $obj['y'] ?? 0;
        $type = $obj['type'] ?? '';
        ...
        $noiDung = match ($type) {
            'nam' => (function () use ($mauFill, $kichThuoc) {
                $ty = self::tySo('nam', $kichThuoc);
                $rNgoai = round(16 * $ty, 2);
                $rTrong = round(5 * $ty, 2);

                return '<circle r="'.$rNgoai.'" fill="'.$mauFill.'"></circle><circle r="'.$rTrong.'" fill="#111111"></circle>';
            })(),
            'con' => (function () use ($mauFill, $kichThuoc) {
                $ty = self::tySo('con', $kichThuoc);
                $nua = round(25 * $ty, 2);
                $canh = round(50 * $ty, 2);
            ...
            'bong' => (function () use ($kichThuoc) {
                $ty = self::tySo('bong', $kichThuoc);
                $nua = round(16 * $ty, 2);
                $canh = round(32 * $ty, 2);
            ...
            'nguoi' => (function () use ($mauFill, $kichThuoc) {
            ...
            'giaovien' => (function () use ($kichThuoc) {
                ...
                return '<circle r="'.$r.'" fill="#111111"></circle><text y="'.round(6 * $ty, 2).'" font-size="'.$fs.'" fill="#ffffff" text-anchor="middle">C</text>';
            })(),
            'hotro' => (function () use ($kichThuoc) {
                $ty = self::tySo('nhansu', $kichThuoc);
                $r = round(16 * $ty, 2);
                $fs = round(16 * $ty, 2);

                return '<circle r="'.$r.'" fill="#111111"></circle><text y="'.round(6 * $ty, 2).'" font-size="'.$fs.'" fill="#ffffff" text-anchor="middle">A</text>';
            })(),
            default => '',
        };
