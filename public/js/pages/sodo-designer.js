(function () {
    ...
    function tySoKichThuoc(nhom) {
        const bang = window.__gaKichThuoc || {};
        const gtri = bang[nhom];
        return (gtri === undefined || gtri === null ? 100 : gtri) / 100;
    }

    function noiDungHinh(type, color) {
        switch (type) {
            case 'nam': {
                const ty = tySoKichThuoc('nam');
                const rNgoai = (16 * ty).toFixed(2);
                const rTrong = (5 * ty).toFixed(2);
                return '<circle r="' + rNgoai + '" fill="' + mauSac(color) + '"></circle>'
                    + '<circle r="' + rTrong + '" fill="#111111"></circle>';
            }
            case 'con': {
                const ty = tySoKichThuoc('con');
                const nua = (25 * ty).toFixed(2);
                const canh = (50 * ty).toFixed(2);
                return '<svg x="-' + nua + '" y="-' + nua + '" width="' + canh + '" height="' + canh + '" viewBox="0 0 16 16">'
            ...
            case 'bong': {
                const ty = tySoKichThuoc('bong');
                const nua = (16 * ty).toFixed(2);
                const canh = (32 * ty).toFixed(2);
                return '<svg x="-' + nua + '" y="-' + nua + '" width="' + canh + '" height="' + canh + '" viewBox="0 0 64 64">'
            ...
            case 'giaovien': {
                ...
                const fs = (16 * ty).toFixed(2);
                const yText = (6 * ty).toFixed(2);
                return '<circle r="' + r + '" fill="#111111"></circle>'
                    + '<text y="' + yText + '" font-size="' + fs + '" fill="#ffffff" text-anchor="middle">C</text>';
            }
            case 'hotro': {
                const ty = tySoKichThuoc('nhansu');
                const r = (16 * ty).toFixed(2);
                ...
                const yText = (6 * ty).toFixed(2);
                return '<circle r="' + r + '" fill="#111111"></circle>'
                    + '<text y="' + yText + '" font-size="' + fs + '" fill="#ffffff" text-anchor="middle">A</text>';
            }
            default:
                return '';
        }
    ...
    gaSizeSliders.forEach(function (slider) {
        const nhom = slider.dataset.group;
        const valueEl = document.getElementById(
            'gaSize' + nhom.charAt(0).toUpperCase() + nhom.slice(1) + 'Value'
        );
        const previewEl = document.querySelector('.ga-size-scale[data-group="' + nhom + '"]');

        slider.addEventListener('input', function () {
            const ty = (parseInt(slider.value, 10) || 100) / 100;
            if (valueEl) valueEl.textContent = slider.value + '%';
            if (previewEl) previewEl.setAttribute('transform', 'scale(' + ty + ')');
        });
    });

    function goiLuu(url, duLieu) {
        return fetch(url, {
            method: 'POST',
            headers: {
    ...
    if (gaMauSacSaveBtn) {
        gaMauSacSaveBtn.addEventListener('click', function () {
            ...
            const duLieuKichThuoc = {};
            gaSizeSliders.forEach(function (slider) {
                duLieuKichThuoc[slider.dataset.group] = parseInt(slider.value, 10) || 100;
            });

            gaMauSacSaveBtn.disabled = true;

            Promise.all([
                goiLuu(window.__gaMauSacUrl, duLieuMau),
                goiLuu(window.__gaKichThuocUrl, duLieuKichThuoc),
            ])
                .then(function (ketQua) {
                    const jsonMau = ketQua[0];
                    const jsonKichThuoc = ketQua[1];
                    if (!jsonMau || !jsonMau.success || !jsonKichThuoc || !jsonKichThuoc.success) {
                        if (typeof showToast === 'function') {
                            showToast('Lưu cài đặt vật dụng thất bại, vui lòng thử lại', 'error');
                        }
                        return;
                    }

                    window.__gaMauSac = jsonMau.mau_sac;
                    window.__gaKichThuoc = jsonKichThuoc.kich_thuoc;
                    capNhatMauPalette(jsonMau.mau_sac);
                    veLaiTatCaSauKhiDoiMau();
                    closeModal('gaMauSacModal');

                    if (typeof showToast === 'function') {
                        showToast('Đã lưu cài đặt màu & kích thước thành công');
                    }
                })
                .catch(function () {
                    if (typeof showToast === 'function') {
                ...
                .finally(function () {
                    gaMauSacSaveBtn.disabled = false;
                });
        });
    }
